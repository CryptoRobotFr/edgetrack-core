...
export default function ApiKeySelector({
  ...
  const handleCreateKey = async () => {
    if (!exchangeName || !keyName || !publicKey) return
    if (!isHyperliquid && !secretKey) return
    if (showPassphrase && !passphrase) return
    if (showMemo && !memo) return
    ...
    try {
      ...
      if (result) {
        toast.success("API key created", { description: `"${keyName}" has been created.` })
